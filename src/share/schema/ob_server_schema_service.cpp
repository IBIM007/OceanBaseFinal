...
int ObServerSchemaService::my_tenant_refresh_schema(
    ...
    const ObRefreshSchemaStatus &schema_status,ObIArray<ObTableSchema> &table_schemas)
{
  ...
  if (!check_inner_stat()) {
    ...
    FLOG_INFO("[REFRESH_SCHEMA] start to refresh full schema",
              "current schema_version", schema_mgr_for_cache->get_schema_version(), K(schema_status));
    LOG_WARN("马上开始我的tenant refresh full schema failed", K(ret), K(schema_status));
    if (OB_FAIL(my_refresh_full_schema(schema_status,table_schemas))) {
      LOG_WARN("tenant refresh full schema failed", K(ret), K(schema_status));
    }

    FLOG_INFO("[REFRESH_SCHEMA] finish mymymymymy refresh full schema", K(ret), K(schema_status),
              "current schema_version", schema_mgr_for_cache->get_schema_version(),
              "cost", ObTimeUtility::current_time() - start);
...
int ObServerSchemaService::my_tenant_refresh_full_schema(
    const ObRefreshSchemaStatus &schema_status,ObIArray<ObTableSchema> &table_schemas)
{
  int ret = OB_SUCCESS;
  const uint64_t tenant_id = schema_status.tenant_id_;
      ...
      while (OB_SUCC(ret) && (core_schema_change || sys_schema_change)) {
        ...
        if (OB_SUCC(ret) && core_schema_change) {
          if (OB_FAIL(schema_service_->get_core_version(
            ...
            } else if (OB_FAIL(my_tenant_try_fetch_publish_core_schemas(schema_status, core_schema_version,
        ...
        if (OB_SUCC(ret) && !core_schema_change && sys_schema_change) {
          if (OB_FAIL(get_schema_version_in_inner_table(sql_client, schema_status, schema_version))) {
            ...
            } else if (OB_FAIL(my_tenant_try_fetch_publish_sys_schemas(schema_status,
                                                             schema_version,
                                                             publish_version,
                                                             sql_client,
        ...
        if (OB_SUCC(ret) && !core_schema_change && !sys_schema_change) {
          LOG_ERROR("马上调用my_refresh_tenant_full_normal_schema了", KR(ret));
          ...
          else if (OB_FAIL(my_tenant_refresh_tenant_full_normal_schema(sql_client, schema_status, fetch_version,table_schemas))) {
      ...
      while (OB_SUCC(ret) && (core_schema_change || sys_schema_change)) {
        ...
        if (OB_SUCC(ret) && !core_schema_change && sys_schema_change) {
          if (OB_FAIL(get_schema_version_in_inner_table(sql_client, schema_status, schema_version))) {
            ...
            if (OB_FAIL(ObSchemaService::gen_sys_temp_version(sys_formal_version, publish_version))) {
              LOG_WARN("gen_sys_temp_version failed", KR(ret), K(schema_status), K(sys_formal_version));
            } else if (OB_FAIL(my_try_fetch_publish_sys_schemas(schema_status,
                                                             schema_version,
                                                             publish_version,
                                                             sql_client,
                                                             sys_schema_change,table_schemas))) {
        ...
        if (OB_SUCC(ret) && !core_schema_change && !sys_schema_change) {
          ...
          //这里调用的refresh_tenant_full_normal_schema
          else if (OB_FAIL(my_refresh_tenant_full_normal_schema(sql_client, schema_status, fetch_version,table_schemas))) {
            LOG_WARN("refresh_full_normal_schema failed", KR(ret), K(schema_status), K(fetch_version));
          } else {
            const int64_t publish_version = std::max(core_schema_version, schema_version);
            schema_mgr_for_cache->set_schema_version(publish_version);
  ...
  if (!check_inner_stat()) {
    ...
    while (OB_SUCC(ret)) {
      ...
      while (OB_SUCC(ret) && (core_schema_change || sys_schema_change)) {
        ...
        // refresh full normal schema by schema_version
        if (OB_SUCC(ret) && !core_schema_change && !sys_schema_change) {
          //LOG_ERROR("马上调用refresh_tenant_full_normal_schema了", KR(ret));
          const int64_t fetch_version = std::max(core_schema_version, schema_version);
          if (OB_FAIL(schema_mgr_for_cache_map_.get_refactored(tenant_id, schema_mgr_for_cache))) {
            LOG_WARN("fail to get schema_mgr_for_cache", KR(ret), K(schema_status));
          } else if (OB_ISNULL(schema_mgr_for_cache)) {
...
int ObServerSchemaService::my_tenant_try_fetch_publish_core_schemas(
    const ObRefreshSchemaStatus &schema_status,
    ...
    bool &core_schema_change,ObIArray<ObTableSchema> &table_schemas)
{
  ...
  if (!check_inner_stat()) {
    ...
    /*if (OB_FAIL(schema_service_->get_core_table_schemas(
      ...
      LOG_ERROR("最新的my_table_schemas是这么多",KR(ret),K(table_schemas.count()));
      for (int64_t i = 0; i < table_schemas.count() && OB_SUCC(ret); ++i) {
        if(table_schemas.at(i).is_sys_table_schema==1)
        {
          if (OB_FAIL(core_tables.push_back(&table_schemas.at(i)))) 
          {
          LOG_WARN("add table schema failed", KR(ret), K(schema_status));
          }
          if (OB_FAIL(core_schemas.push_back(table_schemas.at(i)))) 
          {
          LOG_WARN("add table schema failed", KR(ret), K(schema_status));
          }
        }
      ...
      LOG_ERROR("最新的刷新从磁盘读取这么多条core数据",KR(ret),K(core_tables.count()));
      if (OB_SUCC(ret)) {
        ObSchemaMgr *schema_mgr_for_cache = NULL;
    ...
    bool &core_schema_change,ObIArray<ObTableSchema> &table_schemas)
{
  ...
  if (!check_inner_stat()) {
    ...
    /*if (OB_FAIL(schema_service_->get_core_table_schemas(
      ...
      // core schema don't change, publish core schemas
      ObArray<ObTableSchema *> core_tables;
      LOG_ERROR("my_table_schemas是这么多",KR(ret),K(table_schemas.count()));
      for (int64_t i = 0; i < table_schemas.count() && OB_SUCC(ret); ++i) {
      ...
      LOG_ERROR("刷新从磁盘读取这么多条core数据",KR(ret),K(core_tables.count()));
      if (OB_SUCC(ret)) {
        ObSchemaMgr *schema_mgr_for_cache = NULL;
        auto attr = SET_USE_500("PubCoreSchema", ObCtxIds::SCHEMA_SERVICE);
        ObArenaAllocator allocator(attr);
    ...
    bool &core_schema_change)
{
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_FAIL(schema_service_->get_core_table_schemas(
      ...
      LOG_ERROR("原版的刷新从磁盘读取这么多条core数据",KR(ret),K(core_tables.count()));
      if (OB_SUCC(ret)) {
        ObSchemaMgr *schema_mgr_for_cache = NULL;
        auto attr = SET_USE_500("PubCoreSchema", ObCtxIds::SCHEMA_SERVICE);
        ObArenaAllocator allocator(attr);
...
int ObServerSchemaService::my_tenant_try_fetch_publish_sys_schemas(
    const ObRefreshSchemaStatus &schema_status,
    const int64_t schema_version,
    const int64_t publish_version,
    common::ObISQLClient &sql_client,
    bool &sys_schema_change,ObIArray<ObTableSchema> &table_schemas)
{
  ...
  if (!check_inner_stat()) {
    ...
    {
      if(table_schemas.at(i).is_sys_table_schema==2)sys_schemas.push_back(&table_schemas.at(i));
    }
    LOG_ERROR("最新的刷新从磁盘读取这么多条sys数据",KR(ret),K(sys_schemas.count()));
    ...
    bool &sys_schema_change,ObIArray<ObTableSchema> &table_schemas)
{
  ...
  if (!check_inner_stat()) {
    ...
    LOG_ERROR("刷新从磁盘读取这么多条sys数据",KR(ret),K(sys_schemas.count()));
    ObArray<uint64_t> sys_table_ids;
    int64_t new_schema_version = 0;
    if (OB_FAIL(get_sys_table_ids(tenant_id, sys_table_ids))) {
      LOG_WARN("get sys table ids failed", KR(ret), K(schema_status));
    ...
    bool &sys_schema_change)
{
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_FAIL(get_sys_table_ids(tenant_id, sys_table_ids))) {
      ...
      LOG_ERROR("原版的刷新从磁盘读取这么多条sys数据",KR(ret),K(sys_schemas.count()));
      ObSchemaMgr *schema_mgr_for_cache = NULL;
      auto attr = SET_USE_500("PubSysSchema", ObCtxIds::SCHEMA_SERVICE);
      ObArenaAllocator allocator(attr);
      ObArray<ObSimpleTableSchemaV2*> simple_sys_schemas(
...
int ObServerSchemaService::my_tenant_refresh_tenant_full_normal_schema(
    ObISQLClient &sql_client,
    const ObRefreshSchemaStatus &schema_status,
    const int64_t schema_version,common::ObIArray<share::schema::ObTableSchema> &table_schemas)
{
  ...
  LOG_ERROR("最新的进入my_refresh_tenant_full_normal_schema", K(tenant_id),K(ret));
  ObSchemaMgr *schema_mgr_for_cache = NULL;
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      for(int i=0;i<table_schemas.count();++i)
      {
        //实际上这个感觉都不需要判断，效率应该会高一些。
        if(table_schemas.at(i).is_sys_table_schema!=0)simple_tables.push_back(&table_schemas.at(i));
      }
      LOG_ERROR("最新的my_get_all_tables拿到的schema个数是，总的schema个数是",K(simple_tables.count()),K(table_schemas.count()),K(ret));
      ...
      /*else if (OB_SUCC(schema_service_->get_all_tables(
          sql_client, allocator, schema_status, schema_version, tenant_id, simple_tables))) {
    ...
    if (OB_SUCC(ret)) {
      ...
      for(int i=0;i<table_schemas.count();++i)
      {
        int index=table_schemas.at(i).is_sys_table_schema;
        if(index==3||index==4)non_sys_tables.push_back(&table_schemas.at(i));
      }
      ...
      /*else if (OB_SUCC(schema_service_->get_batch_table_schema(
                 schema_status, schema_version, non_sys_table_ids, sql_client,
                 allocator, non_sys_tables))) {
      ...
      LOG_ERROR("最新的my_normal_full_schema拿到的schema个数是，总的schema个数是",K(non_sys_tables.count()),K(table_schemas.count()),K(ret));
    }
  }
  return ret;
}

int ObServerSchemaService::my_refresh_tenant_full_normal_schema(
    ObISQLClient &sql_client,
    const ObRefreshSchemaStatus &schema_status,
    const int64_t schema_version,common::ObIArray<share::schema::ObTableSchema> &table_schemas)
{
  int ret = OB_SUCCESS;
  uint64_t tenant_id = schema_status.tenant_id_;
  LOG_ERROR("进入my_refresh_tenant_full_normal_schema", K(tenant_id),K(ret));
  ObSchemaMgr *schema_mgr_for_cache = NULL;
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      for(int i=0;i<table_schemas.count();++i)
      {
        simple_tables.push_back(&table_schemas.at(i));
      }
      LOG_ERROR("my_get_all_tables拿到的schema个数是，总的schema个数是",K(simple_tables.count()),K(table_schemas.count()),K(ret));
      if (OB_FAIL(schema_service_->get_sys_variable(sql_client, schema_status, tenant_id,
          schema_version, simple_sys_variable))) {
        LOG_WARN("get all sys variables failed", K(ret), K(schema_version), K(tenant_id));
      } else if (OB_FAIL(schema_service_->get_all_users(
    ...
    if (OB_SUCC(ret)) {
      ...
      {
        int index=table_schemas.at(i).is_sys_table_schema;
        if(index!=0&&index!=1)non_sys_tables.push_back(&table_schemas.at(i));
      }
      if (OB_FAIL(schema_mgr_for_cache->get_non_sys_table_ids(tenant_id, non_sys_table_ids))) {
        LOG_WARN("fail to get non sys table_ids", KR(ret), K(schema_status));
      } 
      ...
      LOG_ERROR("my_normal_full_schema拿到的schema个数是，总的schema个数是",K(non_sys_tables.count()),K(table_schemas.count()),K(ret));
    }
  }
  return ret;
}
    ...
    const int64_t schema_version)
{
  int ret = OB_SUCCESS;
  uint64_t tenant_id = schema_status.tenant_id_;
  //LOG_ERROR("进入refresh_tenant_full_normal_schema", K(tenant_id),K(ret));
  ObSchemaMgr *schema_mgr_for_cache = NULL;
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      LOG_ERROR("原版get_all_tables拿到的schema个数是",K(simple_tables.count()),K(ret));
      if (OB_SUCC(ret)) {
        const ObSimpleTableSchemaV2 *tmp_table = NULL;
        if (OB_FAIL(schema_mgr_for_cache->get_table_schema(tenant_id, OB_ALL_RLS_POLICY_HISTORY_TID, tmp_table))) {
          LOG_WARN("fail to get table schema", KR(ret), K(tenant_id));
    ...
    if (OB_SUCC(ret)) {
      ...
      LOG_ERROR("原版在normal_full_schema里面拿到的batch_table_schema个数是",K(non_sys_tables.count()),K(ret)); 
    }
  }
  return ret;
}
