...
int ObMultiVersionSchemaService::my_tenant_refresh_and_add_schema(const ObIArray<uint64_t> &tenant_ids,ObIArray<ObTableSchema> &table_schemas,
    ...
    auto func = [&]() {
      ...
      if (OB_FAIL(ret)) {
        ...
        for (int64_t i = 0; i < tenant_ids.count(); i++) {
          ...
          } else if (OB_SUCCESS != (tmp_ret = my_tenant_refresh_tenant_schema(tenant_id,table_schemas))) {
            LOG_WARN("fail to refresh tenant schema", K(tmp_ret), K(tenant_id));
          }
          if (OB_SUCCESS != tmp_ret && OB_SUCCESS == ret) {
            ret = tmp_ret;
...
int ObMultiVersionSchemaService::my_tenant_refresh_tenant_schema(
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_SUCC(ret) && need_refresh) {
        if (OB_FAIL(my_tenant_refresh_schema(refresh_schema_status,table_schemas))) {
